...
export default {
  ...
  methods: {
    messageHandler(event) {
      ...
      if (receivedData.type === 'getData') {
        console.log('获取mind数据', receivedData.data) // 输出传输过来的数据
        localStorage.setItem('SIMPLE_MIND_MAP_DATA', receivedData.data)
        store.commit('setIsReadonly', receivedData.isPreview)
        this.getData()
        this.init()
      }
      if (receivedData.type === 'getOldData') {
        console.log('获取历史版本数据', receivedData.data) // 输出传输过来的数据
        localStorage.setItem('SIMPLE_MIND_MAP_DATA', receivedData.data)
        store.commit('setIsReadonly', receivedData.isPreview)
        this.getData()
        this.init()
      }
      if (receivedData.type === 'saveData') {
        window.parent.postMessage({ type: 'saveMind', data: this.getData() }, '*')
      }
    },
    handleStartTextEdit() {
